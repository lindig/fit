...
(** [base] reads the next value of type [ty] from the stream and returns it as a
    [value]. The type [ty] is known because the caller knows from the record
    definition the types of all values. Furthermore, the current endianness
...
module Device = struct
  ...
  let manufacturer = function
    | { manufacturer = None; _ } -> None
    | { manufacturer = Some m; _ } ->
        Some
          (match m with
          | 1 -> "garmin"
